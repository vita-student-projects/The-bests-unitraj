...
class Criterion(nn.Module):
    ...
    def nll_loss_multimodes(self, output, data, N_levels, center_gt_final_valid_idx):
        ...
        for l in range(N_levels+1):
            modes_pred = output[f'level_{l}_probability']
            pred = output[f'level_{l}_trajectory'].permute(1, 2, 0, 3)
            mask = data[..., -1]
            ...
            if use_FDEADE_aux_loss:
                adefde_loss = self.l2_loss_fde(pred, data, mask, l)
            else:
