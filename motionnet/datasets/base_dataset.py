...
from motionnet.datasets.types import object_type, polyline_type
from motionnet.datasets import common_utils
from motionnet.datasets.common_utils import get_polyline_dir,find_true_segments,generate_mask,is_ddp,get_kalman_difficulty,get_trajectory_type,interpolate_polyline
from motionnet.utils.visualization import check_loaded_data
...
class BaseDataset(Dataset):
    ...
    def load_data(self):
        ...
        for cnt, data_path in enumerate(self.data_path):
            ...
            if self.config['use_cache'] or is_ddp():
                data_loaded = self.get_data_list(data_usage_this_dataset)
            else:
                if os.path.exists(self.cache_path) and self.config.get('overwrite_cache', False) is False:
                    ...
                    if not self.is_validation:
                        ...
                        summary_list = summary_list[:data_usage_this_dataset]
                    ...
                    data_loaded = [item for sublist in results for item in sublist]
                    # save the data_loaded in cache_path
                    with open(os.path.join(self.cache_path,'file_list.pkl'),'wb') as f:
                        pickle.dump(data_loaded,f)


            print('Loaded {} samples from {}'.format(len(data_loaded)*self.data_chunk_size, data_path))
            self.dataset_idx[dataset_name] = len(self.data_loaded) + np.arange(len(data_loaded))
            self.data_loaded += data_loaded

            if self.config['store_data_in_memory']:
                print('Loading data into memory...')
                for data_path in data_loaded:
    ...
    def process(self, internal_format):
        ...
        if self.config.manually_split_lane:
            ...
            map_polylines_data, map_polylines_mask, map_polylines_center = self.get_map_data(center_objects=center_objects, map_infos=info['map_infos'])
    ...
    def get_data_list(self,data_usage):
        ...
        if os.path.exists(file_list_path):
            ...
            data_loaded = os.listdir(self.cache_path)
            data_loaded = [os.path.join(self.cache_path, x) for x in data_loaded]
            pickle.dump(data_loaded,open(file_list_path,'wb'))
        np.random.shuffle(data_loaded)
        if not self.is_validation:
            data_loaded = data_loaded[:data_usage]
        return data_loaded
    ...
    def get_map_data(self, center_objects, map_infos):
        ...
        map_polylines = np.pad(map_polylines, ((0, 0), (0, num_of_src_polylines - map_polylines.shape[1]), (0, 0), (0, 0)))
        map_polylines_mask = np.pad(map_polylines_mask, ((0, 0), (0, num_of_src_polylines - map_polylines_mask.shape[1]), (0, 0)))
...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=4, shuffle=False, num_workers=0, collate_fn=train_set.collate_fn)
    ...
    # images = []
    # for n,data in tqdm(enumerate(train_loader)):  # Replace 64 with the actual number of data points
    #     plt = check_loaded_data(data['input_dict'], 0)
    #     buf = io.BytesIO()
    ...
    #     if n > 128:
    #         break
    # final_image = concatenate_images(images, 8,16)
    # final_image.show()
